...
sub run{
	...
	my $taxafile = $self->absolute_path("species.tsv");
	...
	my $consensefile = $self->absolute_path("consense.nex");
	my $finaltreefile = $self->absolute_path("final.nex");
	...
	my $success = $self->_run_subcommand( ("taxize",  "-w", $workdir, "-r", $root_taxa, "-o", $taxafile, "-e", "Subspecies", "-w", $workdir, "-l", "taxize.log") );	
	...
	$success = $self->_run_subcommand( ("bbinfer",  "-w", $workdir, "-s", $supermatrixfile, "-t", $taxafile, "-o", $backbonefile, "-i", "examl", "-b", 100, "-l", "bbinfer.log") );	
	...
	my @params = (("bbreroot",  "-w", $workdir, "-b", $backbonefile, "-t", $taxafile, "-o", $rerootedfile, "-l", "bbreroot.log", '-v'));
	push @params, ("-g",  $outgroup) if $outgroup;
	...
	# Step 8: Consense
	$log->info("Running pipeline step #8: smrt consense");
	$success = $self->_run_subcommand( ("consense",  "-w", $workdir, "-t", $chronogramfile, "-o", $consensefile, "-l", "-b", 0.0, "consense.log") );	

	# Step 8: BBdecompose
	$log->info("Running pipeline step #9: smrt bbdecompose");
	$success = $self->_run_subcommand( ("bbdecompose",  "-w", $workdir, "-b", $consensefile, "-a", $alnfile, "-t", $taxafile, "-l", "bbdecompose.log") );	
	...
	$success = $self->_run_subcommand( ("cladegraft",  "-w", $workdir, "-b", $consensefile, "-o", $finaltreefile, "-l", "cladegraft.log") );	
