...
sub run {
	...
	for my $file ( @files ) {
		...
		for my $seq ( @{$matrix->get_entities} ) {
			...
			$service->insert_seq({gi=>$gi, ti=>$ti, acc=>$acc, acc_vers=>$acc_vers, length=>length($unaligned), division=>$division, acc_date=>$acc_date, gbrel=>$gbrel, def=>$def, seq=>$unaligned});			
